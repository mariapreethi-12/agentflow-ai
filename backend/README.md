...
Copy `.env.example` to `.env` and set `OPENAI_API_KEY` to enable real PM/PRD, Architecture, Backend Code Plan, and QA Plan generation through the OpenAI Responses API. `OPENAI_MODEL` defaults to `gpt-4o-mini`.
